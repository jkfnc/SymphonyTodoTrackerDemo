...
function render() {
  ...
  const status = getListStatus(state.todos);
  ...
  elements.statusBadge.dataset.status = status.toLowerCase().replace(/\s+/g, "-");
  elements.emptyState.hidden = filtered.length > 0;
  elements.clearCompleted.disabled = counts.completed === 0;
